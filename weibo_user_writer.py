...
class WeiboUserWriter(DBAccesor):
    ...
    def insert_new_user_into_db(self, info):
        ...
        if cursor.execute(insert_new_user_sql,(
                ...
                info.get('introduction', '').decode('utf-8','ignore').encode("utf-8"), info.get('realname', ''),
                info.get('location', ''), info.get('registration_date', '1970-01-01'),
    ...
    def read_new_user_from_db(self):
        select_new_user_sql = """
            ...
            FROM WeiboTopic t, topicweiborelation twr, weibocomment wc
